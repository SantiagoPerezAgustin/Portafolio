...
const ProjectDetailModal = ({ project, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="modal-content relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-zinc-900 border border-white/8 shadow-2xl"
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <p className="text-xs font-bold uppercase tracking-widest text-zinc-600 mb-3">Stack</p>
            <div className="flex flex-wrap gap-2">
              {project.tech.map((tech, idx) => (
                <span key={idx} className={`text-xs px-2.5 py-1 rounded-lg border ${a.tech}`}>{tech}</span>
              ))}
            </div>
          </div>
